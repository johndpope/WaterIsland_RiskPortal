...
def live_tradegroup_pnl(request):
    ...
    ytd_performance.columns = ['id','Sleeve', 'TradeGroup', 'LongShort', 'InceptionDate', 'EndDate', 'Status', 'YTD($)']
    ...
    portfolio_df = all_portfolios_df[all_portfolios_df['Group'] == 'ARB']
    ...
    cols2include = ['TradeGroup','TICKER','API_IDENTIFIER','ADJ_PX','Qty','MktVal','FX_RATE',
                        'Sleeve','Bucket','AlphaHedge','LongShort','CatalystName','Analyst','NP_SecType','Capital($)',
                        'Capital(%)','BaseCaseNavImpact','OutlierNavImpact']
    join_on_cols = ['TradeGroup','API_IDENTIFIER','Sleeve','Bucket','AlphaHedge','LongShort','CatalystName',
                            'Analyst','NP_SecType']
