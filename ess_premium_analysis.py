...
def premium_analysis_df_OLS(alpha_ticker, peer_ticker_list, calib_data, analyst_upside, analyst_downside, analyst_pt_wic, as_of_dt, price_tgt_dt, metrics, metric2weight, api_host, adjustments_df_bear, adjustments_df_bull, adjustments_df_pt, bear_flag = None, bull_flag = None, pt_flag = None):
    ...
    for metric in metrics:
        ...
        rows.append([metric, ols_result.params.to_dict()] + [peer2coeff[p] for p in peer_ticker_list] + [peer2coeff['Intercept']])
    ...
    df['Peers Multiples @ Price Target Date'] = df['Metric'].apply(lambda m: {p: peer2ptd_multiple[p][m].fillna(0).iloc[-1] for p in peer_ticker_list})
    ...
    if adjustments_df_bear is None:
        ...
        df['Alpha Bear Multiple @ Price Target Date'] = [compute_multiple_from_price(m, analyst_downside, alpha_balance_sheet_df_bear) for (m,price) in zip(df['Metric'], df['Alpha Downside (analyst)'])]
    else:
    ...
    if adjustments_df_bull is None:
        ...
        df['Alpha Bull Multiple @ Price Target Date'] = [compute_multiple_from_price(m, analyst_upside, alpha_balance_sheet_df_bull) for (m,price) in zip(df['Metric'], df['Alpha Upside (analyst)'])]
    else:
    ...
    if adjustments_df_pt is None:
        ...
        df['Alpha PT WIC Multiple @ Price Target Date'] = [compute_multiple_from_price(m, analyst_pt_wic, alpha_balance_sheet_df_ptd) for (m,price) in zip(df['Metric'], df['Alpha PT WIC (analyst)'])]
    else:
        if pt_flag is None:
            ...
            df['Alpha Balance Sheet DataFrame (PT WIC Case)'] =  [alpha_balance_sheet_df_pt]*len(df)
            df['Alpha PT WIC Multiple @ Price Target Date'] = [compute_multiple_from_price(m, analyst_pt_wic, alpha_balance_sheet_df_pt) for (m,price) in zip(df['Metric'], df['Alpha PT WIC (analyst)'])]
        else:
            adjustments = ast.literal_eval(adjustments_df_pt)[0]
            adjustments_df = pd.DataFrame.from_dict(adjustments, orient = 'index')
            adjustments_df = adjustments_df.T
    ...
    df['Premium Bear (%)'] = (((df['Alpha Bear Multiple @ Price Target Date']/df['Alpha Implied Multiple @ Price Target Date'])*100.0)-100.0).astype(float)
    df['Premium PT WIC (%)'] = (((df['Alpha PT WIC Multiple @ Price Target Date']/df['Alpha Implied Multiple @ Price Target Date'])*100.0)-100.0).astype(float)
    df['Premium Bull (%)'] = (((df['Alpha Bull Multiple @ Price Target Date']/df['Alpha Implied Multiple @ Price Target Date'])*100.0)-100.0).astype(float)
    ...
    df['Peers Multiples @ Now'] = df['Metric'].apply(lambda m: {p: peer2now_multiple[p][m].fillna(0).iloc[-1] for p in peer_ticker_list})
    ...
    df['Alpha Downside'] = [compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_bear) for (m,mult) in zip(df['Metric'], df['Alpha Bear Multiple @ Now'])]
    df['Alpha PT WIC'] = [compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_pt) for (m,mult) in zip(df['Metric'], df['Alpha PT WIC Multiple @ Now'])]
    df['Alpha Upside'] = [compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_bull) for (m,mult) in zip(df['Metric'], df['Alpha Bull Multiple @ Now'])]
...
def premium_analysis_df(alpha_ticker, peers, as_of_dt, last_price_target_dt, analyst_upside, analyst_downside, analyst_pt_wic, metrics, metric2weight, metric2stat_rel, api_host, adjustments_df_bear = None, adjustments_df_bull = None, adjustments_df_pt = None, bear_flag = None, bull_flag = None, pt_flag = None):
    ...
    df['Premium Bear (%)'] = (((df['Alpha Bear Multiple @ Price Target Date']/df['Alpha Implied Multiple (mean) @ Price Target Date'])*100.0)-100.0).astype(float)
    df['Premium PT WIC (%)'] = (((df['Alpha PT WIC Multiple @ Price Target Date']/df['Alpha Implied Multiple (mean) @ Price Target Date'])*100.0)-100.0).astype(float)
    df['Premium Bull (%)'] = (((df['Alpha Bull Multiple @ Price Target Date']/df['Alpha Implied Multiple (mean) @ Price Target Date'])*100.0)-100.0).astype(float)
    
    df['Alpha Bear Multiple @ Now'] = (df['Alpha Implied Multiple (mean) @ Now']*(1+(df['Premium Bear (%)']/100.0))).astype(float)
    df['Alpha PT WIC Multiple @ Now'] = (df['Alpha Implied Multiple (mean) @ Now']*(1+(df['Premium PT WIC (%)']/100.0))).astype(float)
    df['Alpha Bull Multiple @ Now'] = (df['Alpha Implied Multiple (mean) @ Now']*(1+(df['Premium Bull (%)']/100.0))).astype(float)
