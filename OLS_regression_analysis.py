...
def multiple_underlying_df(ticker, start_date_yyyymmdd, end_date_yyyymmdd, api_host, fperiod):
    ...
    px = last_elem_or_null(bbgclient.bbgclient.get_timeseries(ticker, 'PX_LAST', start_date_yyyymmdd, end_date_yyyymmdd,
                                                              api_host=api_host))
    mkt_cap = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'CUR_MKT_CAP', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
    ev_component = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'CUR_EV_COMPONENT', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
    eqy_sh_out = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'EQY_SH_OUT', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
    best_ebitda = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_EBITDA', start_date_yyyymmdd, end_date_yyyymmdd,
                                           {'BEST_FPERIOD_OVERRIDE': fperiod}, api_host))
    best_sales = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_SALES', start_date_yyyymmdd, end_date_yyyymmdd,
                                           {'BEST_FPERIOD_OVERRIDE': fperiod}, api_host))
    best_eps = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_EPS', start_date_yyyymmdd, end_date_yyyymmdd,
                                           {'BEST_FPERIOD_OVERRIDE': fperiod}, api_host))
    div_ind_yield = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'DIVIDEND_INDICATED_YIELD', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
    best_opp = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_OPP', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
    best_ni = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_NET_INCOME', start_date_yyyymmdd, end_date_yyyymmdd,
                                           {'BEST_FPERIOD_OVERRIDE': fperiod}, api_host=api_host))
    best_capex = last_elem_or_null(
        bbgclient.bbgclient.get_timeseries(ticker, 'BEST_CAPEX', start_date_yyyymmdd, end_date_yyyymmdd,
                                           api_host=api_host))
...
def multiples_df(ticker, start_date_yyyymmdd, unaffected_date_yyyymmdd, api_host, fperiod, multiples_to_query='ALL'):
    ...
    df = pd.merge(df, pe.reset_index().rename(columns={'index': 'Date', 0: 'P/E'}), how='left',
                  on=['Date']).ffill().bfill()
...
def run_regression_optimal_peers(alpha_ticker, unaffect_dt, lookback_period=120, fperiod="1BF"):
    ...
    alpha_historical_mult_df = multiples_df(alpha_ticker,
                                            slicer.prev_n_business_days(lookback_period, unaff_dt).strftime('%Y%m%d'),
                                            unaff_dt.strftime('%Y%m%d'), api_host,
                                            fperiod)
    peer2historical_mult_df = {
    p: multiples_df(p, slicer.prev_n_business_days(lookback_period, unaff_dt).strftime('%Y%m%d'),
                    unaff_dt.strftime('%Y%m%d'), api_host, fperiod, multiples_to_query='ALL') for p in peer_ticker_list}
    ...
    for metric in metrics:
        if len(alpha_historical_mult_df[~pd.isnull(alpha_historical_mult_df[metric])]) > 0:
            m_df = alpha_historical_mult_df[['Date', metric]].rename(
                columns={metric: ticker2short_ticker[alpha_ticker]})
            ...
            peer_tickers = [p for p in peer_ticker_list if
            ...
            optimal_model_coeff_df = pd.DataFrame(columns=['Peer', 'Coefficient'],
                                                  data=[(optimal_peers2[count], optimal_model_coeff[count + 1]) for
                                                        count in range(0, len(optimal_model_coeff.names) - 1)])
            optimal_model_coeff_df = optimal_model_coeff_df.append(
                {'Peer': 'Intercept', 'Coefficient': optimal_model_coeff[0]}, ignore_index=True)
    ...
    try:
        ...
        overlap_peers = set.intersection(
            *peer_set.values())  # Select overlap group from the optimal regression peers above
        ...
        if overlap_peers != set():
            ...
            for metric in metrics:
                if len(alpha_historical_mult_df[~pd.isnull(alpha_historical_mult_df[metric])]) > 0:
                    m_df2 = alpha_historical_mult_df[['Date', metric]].rename(
                        columns={metric: ticker2short_ticker[alpha_ticker]})
                    for p in overlap_peers:
                        m_df2 = pd.merge(m_df2, peer2historical_mult_df[p][['Date', metric]], how='left',
                                         on='Date').rename(columns={metric: ticker2short_ticker[p]})
                    overlap_peers = [p for p in overlap_peers if
                                     len(m_df[~pd.isnull(m_df[p.split(' ')[0]])]) > 0]  # remove peers with all nulls
                    m_ols_df2 = m_df2[[alpha_ticker.split(' ')[0]] + [t.split(' ')[0] for t in overlap_peers]]
                    # regress a vs. p1,p2,...,pn
                    formula = alpha_ticker.split(' ')[
                                  0] + ' ~. '  # + " + ".join([t.split(' ')[0] for t in overlap_peers])
...
def premium_analysis_df_OLS(alpha_ticker, regression_results, analyst_upside, tgt_dt, api_host, fperiod="1BF"):
    ...
    peer2ptd_multiple = {p: multiples_df(p, slicer.prev_n_business_days(100, price_tgt_dt).strftime('%Y%m%d'),
                                         ...
                                         multiples_to_query='ALL') for p in peers}
    peer2now_multiple = {
    p: multiples_df(p, slicer.prev_n_business_days(100, as_of_dt).strftime('%Y%m%d'), as_of_dt.strftime('%Y%m%d'),
                    api_host, fperiod='1BF', multiples_to_query='ALL') for p in peers}
    alpha_balance_sheet_df_ptd = multiple_underlying_df(alpha_ticker,
                                                        slicer.prev_n_business_days(100, price_tgt_dt).strftime(
                                                            '%Y%m%d'), price_tgt_dt.strftime('%Y%m%d'), api_host,
                                                        fperiod='1BF')
    alpha_balance_sheet_df_now = multiple_underlying_df(alpha_ticker,
                                                        slicer.prev_n_business_days(100, as_of_dt).strftime('%Y%m%d'),
    ...
    df['Peers Multiples DataFrame @ Price Target Date'] = df['Metric'].apply(
        lambda m: pd.DataFrame(columns=['Peer', 'Multiple'],
                               data=[(p, peer2ptd_multiple[p][m].fillna(0).iloc[-1]) for p in peers_d[m]]))
    df['Alpha Implied Multiple @ Price Target Date'] = df['Metric'].apply(
        lambda m: sum([optimal_results[m][p] * peer2ptd_multiple[p][m].fillna(0).iloc[-1] for p in peers_d[m]]) +
    ...
    df['Peers Multiples DataFrame @ Now'] = df['Metric'].apply(lambda m: pd.DataFrame(columns=['Peer', 'Multiple'],
                                                                                      data=[(p, peer2now_multiple[p][
                                                                                          m].fillna(0).iloc[-1]) for p
                                                                                            in peers_d[m]]))
    df['Alpha Implied Multiple @ Now'] = df['Metric'].apply(
        lambda m: sum([optimal_results[m][p] * peer2now_multiple[p][m].fillna(0).iloc[-1] for p in peers_d[m]]) +
                  optimal_results[m]['(Intercept)'])
    ...
    if len(regression_results) == 2:
        ...
        df2['Peers Multiples DataFrame @ Price Target Date'] = df2['Metric'].apply(
            lambda m: pd.DataFrame(columns=['Peer', 'Multiple'],
                                   data=[(p, peer2ptd_multiple[p][m].fillna(0).iloc[-1]) for p in overlap_peers]))
        df2['Alpha Implied Multiple @ Price Target Date'] = df2['Metric'].apply(
            lambda m: sum([overlap_results[m][p] * peer2ptd_multiple[p][m].fillna(0).iloc[-1] for p in overlap_peers]) +
                      overlap_results[m]['Intercept'])
        df2['Alpha Balance Sheet DataFrame @ Price Target Date'] = [alpha_balance_sheet_df_ptd] * len(df2)
        df2['Alpha Unaffected PX @ Price Target Date'] = [
            compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_ptd) for (m, mult) in
            zip(df2['Metric'], df2['Alpha Implied Multiple @ Price Target Date'])]

        df2['Peers Multiples DataFrame @ Now'] = df2['Metric'].apply(
            lambda m: pd.DataFrame(columns=['Peer', 'Multiple'],
                                   data=[(p, peer2now_multiple[p][m].fillna(0).iloc[-1]) for p in overlap_peers]))
        df2['Alpha Implied Multiple @ Now'] = df2['Metric'].apply(
            lambda m: sum([overlap_results[m][p] * peer2now_multiple[p][m].fillna(0).iloc[-1] for p in overlap_peers]) +
                      overlap_results[m]['Intercept'])
        df2['Alpha Balance Sheet DataFrame @ Now'] = [alpha_balance_sheet_df_now] * len(df2)
        df2['Alpha Unaffected PX @ Now'] = [compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_now) for
                                            (m, mult) in zip(df2['Metric'], df2['Alpha Implied Multiple @ Now'])]

        df2['Alpha Upside (analyst)'] = analyst_upside
        df2['Premium'] = (
                    df2['Alpha Upside (analyst)'].astype(float) - df2['Alpha Unaffected PX @ Price Target Date'].astype(
                float)).fillna(0).apply(lambda x: max(x, 0))
        ...
        optimal_peer_results = {df['Metric'][i]: {'Alpha Downside': df['Alpha Downside (Adj,weighted)'][i],
                                                  'Alpha Upside': df['Alpha Upside (Adj,weighted)'][i]} for i in
                                range(0, len(df))}

        overlap_peer_results = {df2['Metric'][i]: {'Alpha Downside': df2['Alpha Downside (Adj)'][i],
                                                   'Alpha Upside': df2['Alpha Upside (Adj)'][i]} for i in
                                range(0, len(df2))}
...
def premium_analysis_df_OLS_quick(dataframes, regression_results, alpha_ticker, analyst_upside, tgt_dt, api_host,
                                  fperiod="1BF"):
    metrics = ['P/E', "EV/EBITDA", "EV/Sales", 'DVD Yield', 'FCF Yield']
    slicer = dfutils.df_slicer()
    price_tgt_dt = datetime.datetime.strptime(tgt_dt, '%Y-%m-%d')
...
def premium_analysis_df_OLS2(alpha_ticker, regression_results, analyst_upside, analyst_downside, analyst_pt_wic, tgt_dt,
                             api_host, adjustments_df_now=None, adjustments_df_ptd=None, fperiod="1BF"):
    metrics = ['P/E', "EV/EBITDA", "EV/Sales", 'DVD Yield', 'FCF Yield']
    slicer = dfutils.df_slicer()
    price_tgt_dt = datetime.datetime.strptime(tgt_dt, '%Y-%m-%d')
    as_of_dt = datetime.datetime.today()
    ...
    if len(regression_results) == 2:
        ...
        if adjustments_df_ptd is None:
            ...
            df2['Alpha Unaffected PX @ Price Target Date'] = [
                compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_ptd_adj) for (m, mult) in
                zip(df['Metric'], df2['Alpha Implied Multiple @ Price Target Date'])]
        ...
        if adjustments_df_now is None:
            df2['Alpha Balance Sheet DataFrame @ Now'] = [alpha_balance_sheet_df_now] * len(df2)
            df2['Alpha Unaffected PX @ Now'] = [compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_now)
                                                for (m, mult) in
                                                zip(df2['Metric'], df2['Alpha Implied Multiple @ Now'])]
        else:
            adjustments_df1 = adjustments_df_now.drop(columns='Date')
            alpha_balance_sheet_df_now_adj = alpha_balance_sheet_df_now.add(adjustments_df1, axis='columns')
            df2['Alpha Balance Sheet DataFrame @ Now'] = [alpha_balance_sheet_df_now_adj] * len(df)
            df2['Alpha Unaffected PX @ Now'] = [
                compute_implied_price_from_multiple(m, mult, alpha_balance_sheet_df_now_adj) for (m, mult) in
                zip(df['Metric'], df2['Alpha Implied Multiple @ Now'])]
        ...
        df2['Premium Up ($)'] = (
                ...
                float)).fillna(0)
        df2['Premium PT WIC ($)'] = (
                    df2['Alpha PT WIC (analyst)'].astype(float) - df2['Alpha Unaffected PX @ Price Target Date'].astype(
                float)).fillna(0)
        df2['Premium Down ($)'] = (df2['Alpha Downside (analyst)'].astype(float) - df2[
            'Alpha Unaffected PX @ Price Target Date'].astype(float)).fillna(0)

        df2['Alpha Downside (Adj)'] = df2['Alpha Unaffected PX @ Now'].astype(float) + df2['Premium Down ($)'].astype(
            float)
        df2['Alpha PT WIC (Adj)'] = df2['Alpha Unaffected PX @ Now'].astype(float) + df2['Premium PT WIC ($)'].astype(
            float)
        df2['Alpha Upside (Adj)'] = df2['Alpha Unaffected PX @ Now'].astype(float) + df2['Premium Up ($)'].astype(float)

        optimal_peer_results = {
        df['Metric'][i]: {'Alpha Downside': df['Alpha Downside (Adj)'][i], 'Alpha PT WIC': df['Alpha PT WIC (Adj)'][i],
                          'Alpha Upside': df['Alpha Upside (Adj)'][i]} for i in range(0, len(df))}

        overlap_peer_results = {df2['Metric'][i]: {'Alpha Downside': df2['Alpha Downside (Adj)'][i],
                                                   'Alpha PT WIC': df2['Alpha PT WIC (Adj)'][i],
                                                   'Alpha Upside': df2['Alpha Upside (Adj)'][i]} for i in
                                range(0, len(df2))}
...
def premium_analysis_df_OLS2_quick(dataframes, regression_results, alpha_ticker, analyst_upside, analyst_downside,
                                   analyst_pt_wic, tgt_dt, api_host, adjustments_df_now=None, adjustments_df_ptd=None,
                                   fperiod="1BF"):
    metrics = ['P/E', "EV/EBITDA", "EV/Sales", 'DVD Yield', 'FCF Yield']
    slicer = dfutils.df_slicer()
    price_tgt_dt = datetime.datetime.strptime(tgt_dt, '%Y-%m-%d')
