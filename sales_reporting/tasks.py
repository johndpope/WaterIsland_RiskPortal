...
def email_weekly_sales_report():
    ...
    aed_buckets_performance['EndDate'] = aed_buckets_performance['EndDate'].apply(lambda x: x if x is None else pd.to_datetime(x).strftime('%Y-%m-%d'))
    aed_buckets_performance['InceptionDate'] = aed_buckets_performance['InceptionDate'].apply(lambda x: x if x is None else pd.to_datetime(x).strftime('%Y-%m-%d'))
    ...
    del aed_buckets_performance['Metrics in NAV JSON']; del aed_buckets_performance['Metrics in NAV notes JSON']
    del aed_buckets_performance['Metrics in Bet JSON']; del aed_buckets_performance['Metrics in Bet notes JSON']
